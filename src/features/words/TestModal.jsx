...
<div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-700 bg-gray-800">
  ...
  <h2 className="text-lg sm:text-xl font-bold text-white text-center flex-1 mx-2">
    ...
    {testStage === 'testing' && `Тест - ${selectedMode === 'all' ? 'Все слова' : `Стр. ${currentPage}`}`}
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="bg-gray-800 rounded-lg p-3 sm:p-4 border border-gray-700">
                <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center">
                  <div>
                    <p className="text-xs sm:text-sm text-gray-400">Всего слов</p>
                    <p className="text-lg sm:text-xl font-bold text-white">{weekWords.length}</p>
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm text-gray-400">На странице</p>
                  ...
                  <div>
                    ...
                    <p className="text-lg sm:text-xl font-bold text-green-400">{totalPages}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-800 rounded-lg p-3 sm:p-4 border border-gray-700">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="text-left">
                        <div className="font-semibold text-sm sm:text-base">Все слова</div>
                ...
                <div className="relative mb-4">
                  ...
                  <div className="relative flex justify-center">
                    <span className="bg-gray-800 px-2 text-sm text-gray-400">или выберите страницу</span>
                  </div>
                </div>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold text-sm sm:text-base">{page}</div>
                        <div className="text-xs text-gray-400 mt-1">
                          слов: {page === totalPages ? weekWords.length % wordsPerPage || wordsPerPage : wordsPerPage}
                        </div>
                      </div>
                      {selectedMode === 'page' && currentPage === page && (
              ...
              <div className="bg-gray-800 rounded-lg p-3 sm:p-4 border border-gray-700">
                <h4 className="text-sm font-semibold text-white mb-2 text-center">
                  {selectedMode === 'all' ? `Все слова (${weekWords.length})` : `Страница ${currentPage} (${getCurrentPageWords().length} слов)`}
                </h4>
                <div className="space-y-2 max-h-32 sm:max-h-40 overflow-y-auto">
                  {(selectedMode === 'all' ? weekWords.slice(0, 10) : getCurrentPageWords()).map((word, index) => (
                    <div
                      key={word._id}
                    ...
                    >
                      <div className="flex justify-between items-center">
                        ...
                        <div className="text-xs text-gray-500 ml-2 flex-shrink-0">
                          #{selectedMode === 'all' ? index + 1 : (currentPage - 1) * wordsPerPage + index + 1}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-blue-900/30 border border-blue-700 rounded-lg p-3 sm:p-4">
                <h3 className="font-semibold text-blue-300 mb-1 text-sm">Как работает тест:</h3>
              ...
              <div className="flex justify-center">
                <button
                  ...
                  disabled={weekWords.length === 0 || loading || getTestWords().length === 0}
                ...
                >
                  {loading ? 'Загрузка...' : 
                   weekWords.length === 0 ? 'Нет слов для теста' : 
                   selectedMode === 'all' 
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="text-xs text-gray-400">
                  {currentTest.mode === 'all' ? 'Все слова' : `Стр. ${currentTest.page}`}
                </div>
              </div>

              <div className="bg-gray-800 border-2 border-gray-700 rounded-lg p-4 sm:p-6 text-center">
                <div className="text-xs sm:text-sm text-gray-400 mb-2">Переведите на английский:</div>
              ...
              <div className="flex justify-between gap-2">
                ...
                >
                  {currentQuestionIndex === currentTest.words.length - 1 ? 'Завершить' : 'Далее'}
                </button>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="text-center">
                ...
                <p className="text-xs sm:text-sm text-gray-400">
                  {testResults.mode === 'all' ? 'Все слова' : `Стр. ${testResults.page}`} • {testResults.total} слов
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4">
                <div className="bg-green-900/30 border border-green-700 rounded-lg p-2 sm:p-4 text-center">
                  <div className="text-lg sm:text-2xl font-bold text-green-300">{testResults.correct}</div>
                ...
                <div className="bg-yellow-900/30 border border-yellow-700 rounded-lg p-2 sm:p-4 text-center">
                  ...
                  <div className="text-xs sm:text-sm text-yellow-400">С ошибкой</div>
                </div>
                <div className="bg-red-900/30 border border-red-700 rounded-lg p-2 sm:p-4 text-center">
                  <div className="text-lg sm:text-2xl font-bold text-red-300">{testResults.incorrect}</div>
              ...
              <div className="bg-gray-800 rounded-lg p-3 sm:p-4 max-h-40 sm:max-h-60 overflow-y-auto border border-gray-700">
                <h4 className="font-semibold mb-2 text-white text-sm sm:text-base">Результаты:</h4>
                <div className="space-y-1">
                  {testResults.details.map((detail, index) => (
                    <div key={index} className={`p-1.5 rounded border text-xs ${
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1">
                            <span className="text-gray-300 truncate">{detail.russian}</span>
                            <span className="text-gray-500">→</span>
                            <span className="text-white truncate">{detail.english}</span>
                          </div>
              ...
              <div className="bg-blue-900/30 border border-blue-700 rounded-lg p-3 sm:p-4">
                <h4 className="font-semibold text-blue-300 mb-1 text-sm">Статистика:</h4>
                <div className="text-blue-200 text-xs space-y-0.5">
                  <p>• Точно: <strong className="text-white">{testResults.correct}</strong></p>
                  <p>• С ошибкой: <strong className="text-white">{testResults.correctWithTolerance - testResults.correct}</strong></p>
                  <p>• Всего: <strong className="text-white">{testResults.correctWithTolerance}</strong>/{testResults.total}</p>
